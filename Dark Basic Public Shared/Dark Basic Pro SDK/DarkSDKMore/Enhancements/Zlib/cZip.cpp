...
const char* const* cZip::GetAllFileNames ( void )
{
	return m_pszFiles.data();
}

const uLong* cZip::GetAllFileSizes ( void )
{
	return m_piFileSizes.data();
}
...
bool cZip::Open ( LPCSTR szFile, LPCSTR szPassword )
{
	...
	for ( unsigned int i = 0; i < m_Info.number_entry; i++ )
	{
		...
		m_piFileSizes.push_back( fileInfo.uncompressed_size );
...
bool cZip::Add ( char* szFilename, char* szFile, BYTE* pData, int iSize )
{
	...
	int compressionLevel = 2; // value from 0 to 9 for zero to max compression
	// switching to Z_BEST_COMPRESSION makes things smaller, but could be slow on some machines, so using default for now
	if ( strlen ( m_szPassword ) > 0 )
		iResult = zipOpenNewFileInZip3 ( m_ZipFile, szFile, &zipInfo, NULL, 0, NULL, 0, NULL, Z_DEFLATED, compressionLevel, 0, 15, 8, Z_DEFAULT_STRATEGY, m_szPassword, CRC );
	else
		iResult = zipOpenNewFileInZip3 ( m_ZipFile, szFile, &zipInfo, NULL, 0, NULL, 0, NULL, Z_DEFLATED, compressionLevel, 0, 15, 8, Z_DEFAULT_STRATEGY, NULL, CRC );
