...
class CEncryptor  
{
	public:
		...
		explicit CEncryptor(DWORD dwUniqueKeyValue);
		virtual ~CEncryptor();

		void EncryptFileUniqueValue(LPSTR pKey);
		bool EncryptFileData(LPSTR filebuffer, DWORD filebuffersize, bool bEncryptIfTrue);
		DWORD GetUniqueKey(void) const { return m_dwUniqueKey; }
		void SetUniqueKey(DWORD dwValue) { m_dwUniqueKey = dwValue; }
		void SetUniqueKeyName(const char* szString) { if (szString) strcpy_s(m_szStringKey, sizeof(m_szStringKey), szString); }

	private:

		DWORD m_dwUniqueKey = 0;
