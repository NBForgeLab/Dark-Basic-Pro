...
{
	return m_tokenizer.ProduceNextTokenEx(pString, bIncrementLineNumber, bProduceCRTK, bIncludeCommas, bIgnoreSpacesAroundEquateSymbol);
}

LPSTR CStatement::ProduceNextToken(LPSTR* pString, bool bIncrementLineNumber, bool bProduceCRTK, bool bIncludeCommas)
{
	return m_tokenizer.ProduceNextToken(pString, bIncrementLineNumber, bProduceCRTK, bIncludeCommas);
}

LPSTR CStatement::ProduceNextArrayToken(LPSTR* pOrigPointer)
{
	return m_tokenizer.ProduceNextArrayToken(pOrigPointer);
}

LPSTR CStatement::ProduceFullSegment(LPSTR* pOrigPointer)
{
	return m_tokenizer.ProduceFullSegment(pOrigPointer);
}
...
bool CASTAssignment::WriteDBM()
{
	...
	ASTPrinter printer;
	DBP_INFO("--- AST Assignment Pipeline Trace (Line {}) ---", m_lineNumber);
	DBP_INFO("Variable: {}", m_varName);
	DBP_INFO("Expression: {}", m_valStr);
